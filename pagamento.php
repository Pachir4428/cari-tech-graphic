 ...
 * Finanças, entrega automaticamente o pedido (se o serviço tiver entrega
 * configurada) e, quando o pedido fica totalmente pago, avisa o cliente por
 ...
 *
 * Blindagem: TODA a lógica corre dentro de um try/catch — se o gateway já
 * tiver confirmado o pagamento (dinheiro já saiu da conta do cliente) mas
 * ocorrer um erro inesperado a seguir, nunca respondemos como se o pagamento
 * tivesse falhado; tentamos registar o essencial (estado "pago") e avisamos
 * com uma mensagem tranquilizadora, nunca um 500 em branco.
 */
...
/* Aplica um pagamento confirmado na hora: regista o valor, a entrada nas
   Finanças, a entrega automática (se configurada) e, se transitar para
   "pago", avisa o cliente por e-mail. Partilhado pelos dois gateways quando
   confirmam de forma síncrona (teste, ou métodos instantâneos). */
function ctg_aplicar_pagamento($config, $lead, $leadId, $email, $codigo, $valor, $ehTotal, $valorTotalDeclarado, $origem, $mensagemGateway) {
    ...
    return [
        'message' => $msg, 'pago' => $pagoAgora, 'valorPago' => $slot['valorPago'] ?? $valor,
        'valorAcordado' => $slot['valorAcordado'] ?? null, 'saldo' => $saldo,
        'instrucoes' => $entrega['instrucoes'] ?? '',
    ];
}

/* Estado partilhado com o catch: assim que soubermos que o gateway confirmou
   o pagamento, isto fica marcado — nunca respondemos como falha depois disto. */
$leadIdSeguranca = '';
$pagamentoGatewayConfirmado = false;
...
try {
    ...
    /* Intervalo de valores permitido (Definições → Pagamento automático). */
    $limites = store_get_limite_valor($config);
    if (!empty($limites['ativo'])) {
        $min = isset($limites['min']) ? (float) $limites['min'] : null;
        $max = isset($limites['max']) ? (float) $limites['max'] : null;
        if (($min !== null && $valor < $min) || ($max !== null && $valor > $max)) {
            $msgLimite = trim((string) ($limites['mensagem'] ?? ''));
            if ($msgLimite === '') {
                $msgLimite = 'O valor deve estar entre ' . number_format($min ?? 0, 2, ',', ' ') . ' e ' . number_format($max ?? 0, 2, ',', ' ') . ' MT.';
            }
            responder(false, ['message' => $msgLimite], 422);
        }
    }
    ...
    if ($pagajaActivo) {
        ...
        // A partir daqui a PagaJá já validou/aceitou a cobrança — nunca respondemos como falha.
        $pagamentoGatewayConfirmado = true;
    ...
    error_log('pagamento.php: ' . $erro->getMessage());
    try {
        store_add_audit($config, 'pagamento-erro', ($leadIdSeguranca ?: '?') . ': ' . $erro->getMessage());
    } catch (\Throwable $e2) { /* nunca deixar o registo do erro derrubar a resposta */ }

    if ($pagamentoGatewayConfirmado && $leadIdSeguranca !== '') {
        // O gateway já confirmou (o dinheiro já saiu da conta do cliente) — tenta
        // pelo menos marcar "pago" com a função mais simples possível, e nunca
        // diz ao cliente que o pagamento falhou.
        try { store_set_pagamento($config, $leadIdSeguranca, 'pago'); } catch (\Throwable $e3) { /* melhor esforço */ }
        responder(true, ['message' => 'Pagamento recebido! Estamos a finalizar o registo do seu pedido — se os ficheiros não aparecerem na Área de Cliente dentro de alguns minutos, contacte-nos pelo WhatsApp.']);
    }
    responder(false, ['message' => 'Não foi possível concluir o pagamento agora. Tente novamente ou contacte-nos pelo WhatsApp.'], 500);
}
